...
#define I2C_MASTER_SDA_IO CONFIG_MA120X0_SDA_PIN /* 0 */ /*!< gpio number for I2C master data  */
...
uint8_t b[32];
#define CHECK(ADDR, L)            \
  ma_read(0x20, 2, ADDR, b, L);   \
  printf("Check 0x%04x :", ADDR); \
...
void ma120_setup_audio(uint8_t i2c_addr)
{
  ...
  cmd[0] = 0x20 /* 0x50 */; //Volume. Tried 0x20 with no success.
  cmd[1] = 0x50;
  cmd[2] = 0x02;
